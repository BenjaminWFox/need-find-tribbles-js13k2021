## Tribble Storage & Minting

### Step 1

Assign file name based on game coordinates

### Step 2

Upload files to AWS s3 for easy preview image

CLI command:
`aws s3 mv test-tribble.png s3://js13k-2021-tribbles-test`

### Step 3

Upload files to NFT.storage (https://nft.storage/) for archival

```bash
# Get key here: https://nft.storage/manage/
curl -X POST --data-binary @test-tribble.png -H "Authorization: Bearer '$NFTSTORAGEKEY'" https://api.nft.storage/upload
```

### Step 4

Mint NFT with s3 preview URL and NFT.storage metadata url.

```bash
near call $ID nft_mint '{  "token_id": "6",  "token_owner_id": "'$ID'", "token_metadata": { "title": "THE COUNT", "description": "One NFT. Ah ah ahh.", "media": "https://benjaminwfox.com/images/blog/tech/thinking-in-nextjs/the-count-3-things-4.jpg", "media_hash": "", "copies": 1, "issued_at": "", "expires_at": "", "starts_at": "",    "updated_at": "", "extra": "", "reference": "", "reference_hash": "" }}' --accountId $ID --deposit .00005
```
